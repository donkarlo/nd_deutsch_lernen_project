...
\begin{document}
   ...
   \section{Hinweis \textnormal{(Note)}}
      ...
      (Der erste Vorteil(m))(subject) ist(mit sein, immer Nominativ), dass die Einkommensschwachen(kam daramad, substantivierte Adjektiv)  auch eine äquivalente Bewegungsmöglichkeit haben können, deshalb ich glaube, dass die Regierung(goverment)  die Reichen mehr besteuren(to tax) muss, damit die Menschen, die niedrigere(Komparativ) Einkommen(neutral)haben, ein effizienteres öffentliches Verkehrssystem erreichen können.
      ...
      Sobald(subordinierende Konjunktion wie "weil", dann das Verb steht nicht vor Subjekt) ich denke an (denken an + AKK) die Vorteile(p), ich erinnere mich (erinnere ist reflexiv, errineren an + Akkusativ) den Nutzen(Akk Objekt) der(Genetiv) öffentlichen(Adjectiv) Verkehrsmittle für die Natur(f).

      Wenn ich mich nicht irre,  errinere Ich mich, dass die Wissenschaftler vor viel Jahren angegaben hatten, dass ein Bus mit Zwenzig Menchen an Bord  die Luft mehrmals weniger verschmutzt als zwinzig Menschen in unabhängig Autos.

      Das bedeutet weniger Gesundheitskosten und folglich längeren Lebenswillen(m und hat kein Plural). Weniger Lärmbelastung ist auch ein anderer(Aus gemichte von ein) Vorteil(m), die(bezieht Lärmbelastung, die Feminin Singular ist)  (bezieht Lärmbelastung) wir  ständig in(Dativ) überfüllten(stark) Städten(Dativ plural meinstens nimmt ein -n am Ende, shaen nom>deklination) zu vermeiden versuchen, die(bezieht Lärmbelastung) durch mehr öffentliche Verkehrsmittel  erreichbarer ist(weil Lärmbelastung ist Singular). 
      
      Das Zussamensein mit den täglischen(Adjectiv schwach) Menchen(ist immer Plural) in die Geselschaft ist andrer(Aus ist, wir musss ein Nominativ Adjective benutzen) Vorteil(masculin) nach die ich denke. Ich personlisch genieße mich so viel wenn ich in städtich veröffentlische Verkehrmittel die Falten der Gesichte die Täglische Menschen wovon ich ihre gedanken . 
