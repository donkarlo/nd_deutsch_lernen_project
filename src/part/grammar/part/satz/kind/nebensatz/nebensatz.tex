...
\begin{document}
    ...
    \section{Verschachtelte Nebensätze \textnormal{(Nested subordinate clauses)}}

        \begin{grammarentry}{Grundprinzip}{basic principle}
            \GermanText{Ein Nebensatz kann einen weiteren Nebensatz enthalten. Jeder Nebensatz bildet dabei seine eigene grammatische Einheit. Deshalb hat jeder Nebensatz sein eigenes konjugiertes Verb, und dieses Verb steht am Ende genau dieses Nebensatzes.}
            \EnglishText{A subordinate clause can contain another subordinate clause. Each subordinate clause forms its own grammatical unit. Therefore, each subordinate clause has its own conjugated verb, and that verb stands at the end of that particular subordinate clause.}

            \begin{exbox}
                \begin{itemize}
                    \ExampleItem{Ich glaube, dass der Mann, der dort arbeitet, sehr freundlich ist.}{I believe that the man who works there is very friendly.}
                    \ExampleItem{Sie sagt, dass sie kommt, wenn sie Zeit hat.}{She says that she will come if she has time.}
                \end{itemize}
            \end{exbox}
        \end{grammarentry}

        \begin{grammarentry}{Relativsatz in einem anderen Nebensatz}{relative clause inside another subordinate clause}
            \GermanText{Ein Relativsatz kann innerhalb eines anderen Nebensatzes stehen. Das Verb des Relativsatzes steht am Ende des Relativsatzes. Danach geht der äußere Nebensatz weiter; sein eigenes konjugiertes Verb steht am Ende des äußeren Nebensatzes.}
            \EnglishText{A relative clause can stand inside another subordinate clause. The verb of the relative clause goes to the end of the relative clause. After that, the outer subordinate clause continues; its own conjugated verb goes to the end of the outer subordinate clause.}

            \begin{exbox}
                \begin{itemize}
                    \ExampleItem{damit die Menschen, die niedrigere Einkommen haben, ein effizienteres öffentliches Verkehrssystem erreichen können}{so that the people who have lower incomes can reach a more efficient public transport system}
                    \ExampleItem{Relativsatz: die niedrigere Einkommen haben}{Relative clause: who have lower incomes; \textit{haben} closes the relative clause.}
                    \ExampleItem{Äußerer Nebensatz: damit die Menschen \ldots{} ein effizienteres öffentliches Verkehrssystem erreichen können}{Outer subordinate clause: so that the people ... can reach a more efficient public transport system; \textit{können} closes the outer \textit{damit}-clause.}
                \end{itemize}
            \end{exbox}
        \end{grammarentry}

        \begin{grammarentry}{So liest man die Struktur}{how to read the structure}
            \GermanText{Beim Lesen verschachtelter Nebensätze sollte man zuerst bestimmen, wo jeder Nebensatz beginnt und endet. Ein eingeschobener Nebensatz unterbricht den äußeren Nebensatz nur vorübergehend. Nach dem Ende des eingeschobenen Nebensatzes wird der äußere Nebensatz fortgesetzt.}
            \EnglishText{When reading nested subordinate clauses, first determine where each subordinate clause begins and ends. An embedded subordinate clause only interrupts the outer clause temporarily. After the embedded clause ends, the outer subordinate clause continues.}

            \begin{exbox}
                \begin{itemize}
                    \ExampleItem{damit die Menschen, [die niedrigere Einkommen haben], ein effizienteres öffentliches Verkehrssystem erreichen können}{The brackets show the embedded relative clause.}
                    \ExampleItem{haben = Ende des Relativsatzes; können = Ende des damit-Satzes}{\textit{haben} = end of the relative clause; \textit{können} = end of the \textit{damit}-clause.}
                \end{itemize}
            \end{exbox}
        \end{grammarentry}
